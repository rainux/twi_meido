module TwiMeido
  module TimelineCommands
    ...
    define_command :retweet, /\A-re\s*(\d+|[a-z]+)\Z/i do |user, message, params|
      id = params[1]
...
Successfully retweeted tweet #{id}, ご主人様.
    ...
    define_command :retweet_with_comment, /\A-rt\s*(\d+|[a-z]+)\s*(.*)\Z/i do |user, message, params|
      id = params[1]
      comment = params[2]
...
Successfully replied to #{in_reply_to_tweet.user.screen_name}'s tweet #{in_reply_to_tweet.id}, ご主人様.
    ...
    define_command :reply_all, /\A-ra\s*(\d+|[a-z]+)\s*(.*)\Z/i do |user, message, params|
      id = params[1]
      status = params[2]
...
Successfully replied to all mentioned users of #{in_reply_to_tweet.user.screen_name}'s tweet #{in_reply_to_tweet.id}, ご主人様.
    ...
    define_command :direct_messages, /\A-d\Z/ do |user, message|
...
#{tweet.sender.screen_name}: #{CGI.unescapeHTML(tweet.text)}
    ...
    define_command :delete, /\A-del\s*(\d+|[a-z]+)?\Z/i do |user, message, params|
      id = params[1]
...
Successfully deleted your tweet #{id}.
    ...
    define_command :show, /\A-show\s*(\d+|[a-z]+)(\s+\d+)?\Z/i do |user, message, params|
      id = params[1]
