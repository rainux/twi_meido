module TwiMeido
  module AccountCommands
    ...
    define_command :oauth, /\Aoauth\Z/ do |user, message|
...
-bind PIN_CODE
    ...
    define_command :bind, /\Abind\s+(.+)\Z/ do |user, message, params|
      begin
        client = TwitterOAuth::Client.new(
          :consumer_key => AppConfig.twitter.consumer_key,
          :consumer_secret => AppConfig.twitter.consumer_secret
...
Please make sure you've turned track on via command -on track.
    ...
    define_command :reset_short_id, /\Areset\Z/ do |user, message|
      user.reset_short_id
      'Short ID reset, ご主人様.'
    end

    define_command :reconnect, /\Areconnect\Z/ do |user, message|
      user.reconnect_user_streams
      'User Streams reconnected, ご主人様.'
    end

    define_command :follow, /\Afo\s+(\S+)\Z/ do |user, message, params|
    ...
    define_command :unfollow, /\Aunfo\s+(\S+)\Z/ do |user, message, params|
      screen_name = params[1]
    ...
    define_command :if_follow, /\Aif\s+(\S+)\Z/ do |user, message, params|
      screen_name = params[1]
